...
describe('hybridSearch — keyword mode', () => {
  test('returns results with expected fields for "authentication"', async () => {
    const r = await hybridSearch('authentication', { mode: 'keyword', maxResults: 5 });
  ...
  test('spec filter constrains results to the requested spec', async () => {
    const r = await hybridSearch('PDN connection', { spec: 'ts_24_301', maxResults: 5 });
  ...
  test('section number query does not throw', async () => {
    ...
    const r = await hybridSearch('5.5.1.2', { mode: 'keyword', maxResults: 3 });
    assert.ok(typeof r === 'object', 'result must be an object');
    assert.ok(Array.isArray(r.results), 'results must be an array');
  });

  test('nonsense query returns 0 results', async () => {
    const r = await hybridSearch('xyzzy_nonexistent_term_abc123', { mode: 'keyword' });
    assert.ok(
...
describe('hybridSearch — edge cases', () => {
  test('empty string returns object without throwing', async () => {
    const r = await hybridSearch('');
    assert.ok(typeof r === 'object', 'Must return an object');
    assert.ok(Array.isArray(r.results), 'results must be an array');
  });

  test('whitespace-only string returns object without throwing', async () => {
    const r = await hybridSearch('   ');
    assert.ok(typeof r === 'object', 'Must return an object');
    assert.ok(Array.isArray(r.results), 'results must be an array');
  });

  test('result object always has required shape', async () => {
    const r = await hybridSearch('authentication', { mode: 'keyword', maxResults: 1 });
...
describe('hybridSearch — scoring and fusion', () => {
  test('includeScores reveals score fields', async () => {
    const r = await hybridSearch('PDN', { mode: 'keyword', maxResults: 5, includeScores: true });
  ...
  test('keyword-only mode: effectiveAlpha is 1.0', async () => {
    const r = await hybridSearch('PDN', { mode: 'keyword', maxResults: 5, includeScores: true });
    assert.ok(r.results.length > 0, 'Expected at least one result');
    const result = r.results[0];
  ...
  test('scores are clamped to [0, 1]', async () => {
    const r = await hybridSearch('PDN', { mode: 'keyword', maxResults: 10, includeScores: true });
    if (r.results.length > 0) {
  ...
  test('mode hybrid falls back to keyword when no sqlite-vec', async () => {
    const r = await hybridSearch('PDN', { mode: 'hybrid', maxResults: 3 });
  ...
  test('mode semantic falls back to keyword', async () => {
    const r = await hybridSearch('PDN', { mode: 'semantic', maxResults: 3 });
...
describe('hybridSearch — title-aware chapter ranking', () => {
  test('registration procedure prefers the canonical procedure chapter over generic leaves', async () => {
    const r = await hybridSearch('registration procedure', {
      spec: 'ts_24_501',
      mode: 'keyword',
      maxResults: 10,
      useCache: false,
  ...
  test('initial registration prefers the chapter anchor and emits navigation hints for generic subsections', async () => {
    const r = await hybridSearch('initial registration', {
      spec: 'ts_24_501',
      mode: 'keyword',
      maxResults: 10,
      useCache: false,
...
describe('hybridSearch — cache integration', () => {
  test('repeated query returns cached result', async () => {
    queryCache.clear();
    queryCache.resetStats();
    
    const r1 = await hybridSearch('PDN connection', { spec: 'ts_24_301', mode: 'keyword', maxResults: 5 });
    const r2 = await hybridSearch('PDN connection', { spec: 'ts_24_301', mode: 'keyword', maxResults: 5 });
  ...
  test('useCache: false bypasses cache', async () => {
    ...
    const r1 = await hybridSearch('authentication', { mode: 'keyword', maxResults: 3 });
    const r2 = await hybridSearch('authentication', { mode: 'keyword', maxResults: 3, useCache: false });
    
    assert.ok(!r2.cached || r2.cached !== true, 'Result with useCache: false should not have cached: true');
  });

  test('cache stats reflect usage', async () => {
    queryCache.clear();
    queryCache.resetStats();
    
    await hybridSearch('authentication', { mode: 'keyword', maxResults: 2 });
    await hybridSearch('authentication', { mode: 'keyword', maxResults: 2 });
    await hybridSearch('PDN', { mode: 'keyword', maxResults: 2 });
    await hybridSearch('PDN', { mode: 'keyword', maxResults: 2 });
  ...
  test('auto mode skips cache to avoid stale semantic fallback reuse', async () => {
    queryCache.clear();
    queryCache.resetStats();

    const r1 = await hybridSearch('authentication', { mode: 'auto', maxResults: 2 });
    const r2 = await hybridSearch('authentication', { mode: 'auto', maxResults: 2 });

    assert.ok(!r1.cached, 'First auto-mode result should not be cached');
    assert.ok(!r2.cached, 'Auto-mode result should not be served from cache');

    const stats = getQueryCacheStats();
    assert.strictEqual(stats.hits, 0, 'Auto mode should not produce cache hits');
  });
});
