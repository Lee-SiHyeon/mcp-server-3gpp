...
import { getSpecById, getSpecSuggestions } from '../db/queries.js';
...
export function handleGetSpecReferences(args) {
  try {
    ...
    if (!spec) {
      // Suggest similar IDs
      const suggestions = getSpecSuggestions(specId);
      return formatError({
    ...
    console.error(`[get_spec_references] Error:`, error.message, { args });
    return formatError({
      error: error.message,
      tool: 'get_spec_references',
      context: { args },
    });
  }
}
