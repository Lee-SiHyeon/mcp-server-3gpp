...
// Helper function to search chunks
function searchChunks(query, specFilter = null, maxResults = 5) {
  ...
  if (specFilter) {
    results = results.filter(chunk => 
      chunk.spec.toLowerCase().includes(specFilter.toLowerCase())
    );
  }
...
runTest('Scenario 1.2: Search returns results with spec names', () => {
  ...
  // Check if we have multiple specs OR at least results from a valid spec
...
runTest('Scenario 2.3: Invalid cause #999 returns fallback to #111', () => {
  // The server returns #111 as fallback for undefined causes
